...
import { getAdminBusiness, getAdminUrgencies, getAdminUser, searchAdminUsers, syncAdminUser } from '../adminApi'
...
function lastCall() {
  ...
  return { url, method: init?.method, headers: (init?.headers ?? {}) as Record<string, string> }
}
